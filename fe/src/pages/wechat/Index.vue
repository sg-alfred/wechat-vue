...
<div class="wechat-page">
  ...
  <main class="wechat-container">
    <wechat-item v-for="item in chatrooms" :key="item.id" :chat-item="item" @into-chatroom="intoChatroom" />
  </main>
...
export default {
  ...
  beforeMount() {
    this.chatrooms = Object.values(this.contacts).filter(
      (ele) =>
      ele.messages.length > 0 ||
      (!!ele.chatinfo.lastmsgid &&
        (!ele.cleartime || ele.chatinfo.sendtime > ele.cleartime))
    );
