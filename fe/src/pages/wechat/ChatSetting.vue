...
<div class="chatSetting-page">
  ...
  <main class="setting-container">
    ...
    <section class="chatRole-section">
      <div>
        <span>置顶聊天</span>
        <el-switch v-model="stickyTop" class="switch-label right" on-color="#13ce66" off-color="grey" />
      </div>
      <div>
        <span>消息免打扰</span>
        <el-switch v-model="muteNoti" class="switch-label right" on-color="#13ce66" off-color="grey" />
      </div>
      <!-- 如果是 群的话，就要多一个选项～ -->
      <!--<div>
            <span>保存到通讯录</span>
...
export default {
  ...
  methods: {
    ...
    async confirmClearHistory() {
      this.$confirm(
          `确定删除和${this.contctInfo.mobilephone}的聊天记录`,
          '提示', {
        ...
        .then(() => {
          ...
          clearChatHistory(this.contctInfo.fid, {
            cleartime
          }).then(
            (response) => {
