...
class User {
  ...
  async loginByFace (uid, base64Img) {
    ...
    const response = await aipFaceClient.verifyUser(String(uid), GROUP_IDS, base64Img, {top_num: 1})
    ...
    try {
      if (updateType === 'avatar') {
        ...
        switch (updateParams.type) {
          case 'add':
            response = await aipFaceClient.addUser(uid, info, GROUP_IDS, updateParams.base64Img)
            ...
            break
          case 'update':
            response = await aipFaceClient.updateUser(uid, info, GROUP_IDS, updateParams.base64Img)
